...
inline size_t get_padding(uint32_t alignment) {
...
inline void *malloc_impl(uint32_t alignment, size_t size) {
  void *ptr = nullptr;
  assert(alignment >= alignof(usm_alloc_info) &&
         "memory not aligned to usm_alloc_info");
#ifdef _MSC_VER
...
inline uint8_t *get_alloc_info_addr(const void *ptr) {
  return (uint8_t *)const_cast<void *>(ptr) - alloc_header_size;
}

inline usm_alloc_info get_alloc_info(void *ptr) {
...
struct ur_context_handle_t_ : RefCounted {
  ...
  void *add_alloc(uint32_t alignment, ur_usm_type_t type, size_t size,
    ...
    *reinterpret_cast<native_cpu::usm_alloc_info *>(info_addr) = info;
  ...
  void remove_alloc(void *ptr) {
    native_cpu::usm_alloc_info info = native_cpu::get_alloc_info(ptr);
#ifdef _MSC_VER
